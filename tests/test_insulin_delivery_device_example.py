...
class TestInsulinDeliveryDeviceExample(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Actors
        self.mfg = Actor(name="Manufacturer", actor_type="Organization", physical_access=False)
        self.dist = Actor(name="Distributor", actor_type="Organization", physical_access=False)
        self.user = Actor(name="End User", actor_type="Individual", physical_access=True)
        ...
        # Processes
        self.idd_controller = Process(name="Insulin Delivery Device Controller")
        ...
        self.idd_therapy_setting_db = Datastore(name="IDD Therapy Setting Data Store", ds_type=DATASTORE_TYPE.SQL)
        self.phd_idd.add_child(self.idd_therapy_setting_db)

        self.idd_observation_db = Datastore(name="IDD Observation Data Store", ds_type=DATASTORE_TYPE.SQL)
        self.phd_idd.add_child(self.idd_observation_db)

        self.connected_configuration_db = Datastore(name="Connected Device Configuration Data Store", ds_type=DATASTORE_TYPE.SQL)
        self.connected_device.add_child(self.connected_configuration_db)

        self.connected_therapy_setting_db = Datastore(name="Connected Device Therapy Setting Data Store", ds_type=DATASTORE_TYPE.SQL)
        self.connected_device.add_child(self.connected_therapy_setting_db)

        self.connected_observation_db = Datastore(name="Connected Device Observation Data Store", ds_type=DATASTORE_TYPE.SQL)
        self.connected_device.add_child(self.connected_observation_db)

        # Dataflows
        self.user_to_connected_controller = DataFlow(name = "User to Connected Device Controller", 
                                                src = self.user, 
                                                dst = self.connected_controller)
        
        self.connected_device_to_user = DataFlow(name="Connected Device Controller to User", 
                                            src=self.connected_controller, 
                                            dst=self.user)

        self.user_to_idd_controller = DataFlow(name="User to Insulin Delivery Device Controller", 
                                          src=self.user, 
                                          dst=self.idd_controller)

        self.idd_controller_to_user = DataFlow(name="Insulin Delivery Device Controller to User", 
                                          src=self.idd_controller, 
                                          dst=self.user)

        self.pump_to_user = DataFlow(name="Actuator Pump Drive to User", 
                                src=self.actuator_pump, 
                                dst=self.user)

        self.mfg_to_idd_controller = DataFlow(name="Mfg to Insulin Delivery Device Controller", 
                                         src=self.mfg, 
                                         dst=self.idd_controller)

        self.idd_controller_to_mfg = DataFlow(name="Insulin Delivery Device Controller to Mfg", 
                                         src=self.idd_controller, 
                                         dst=self.mfg)

        self.idd_controller_to_connected_controller = DataFlow(name="Insulin Delivery Device Controller to Connected Device Controller", 
                                                          src=self.idd_controller, 
                                                          dst=self.connected_controller)
    ...
    def test_find_related_attack_vectors(self):
        ...
        attack_vectors = self.tm.find_related_attack_vectors(self.idd_controller)
