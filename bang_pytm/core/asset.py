...
class Asset(Component):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if (not isinstance(open_ports, list) or not all(isinstance(port, int) for port in open_ports)) and open_ports is not None:
        ...
        if (not isinstance(trust_boundaries, list) or not all(isinstance(boundary, Boundary) for boundary in trust_boundaries)) and trust_boundaries is not None:
            if isinstance(trust_boundaries, Boundary):
                self.boundaries = [trust_boundaries]
            else:
                raise ValueError("Trust Boundaries must be a list of Boundary objects")
        self.boundaries = trust_boundaries
